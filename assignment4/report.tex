...
To implement the photometric stereo, we have created the matrix for the albedo modulated normals as $M = S^{-1} J$, where $S$ is the light sources and $J$ is  the image intensities, where the binary mask that indicates areas where we should solve for photometric stereo, equals 1.\\\\
As the Beethoven image dataset is a synthetic example, we have used the "exact" inverse. This has been computed using the Matlab built-in function for calculation inverse matrices. For the real example of the Buddha image dataset, we have used the pseudo-inverse (as we are not garanteed that the exact inverse exists when using real data). This we have done using the built-in Matlab function \texttt{pinv}.\\\\
