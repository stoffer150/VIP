...
\begin{itemize}
\item[-] The input images are rectified beforehand.
...
Part of the reason why we get so many errors might be because we have calculated disparity on different images than the images used for calculating the ground truth. We used \texttt{scene1.row3.col1.ppm} and \texttt{scene1.row3.col2.ppm} whereas the name of the ground truth image indicates that they have used \texttt{scene1.row3.col3.ppm} together with some other (not stated) image. The values of the ground truth and our calculation might also be scaled a bit differently. Despite the errors, we believe overall that we have successfully provided depth information which mostly consists with what we see in the ground truth image.
\end{homeworkSubProblem}
